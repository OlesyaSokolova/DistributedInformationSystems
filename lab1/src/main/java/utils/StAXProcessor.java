...
import elements_processors.ElementProcessor;
import elements_processors.NodeProcessor;
import elements_processors.TagProcessor;
...
public class StAXProcessor {
    ...
    private static final Map<String, ElementProcessor> processors = new HashMap<>();
    static {
            ...
            while (elementsCounter < limit && reader.hasNext()) {
                ...
                if (nextEvent.isStartElement()) {
                    ...
                    if(startElementName.equals(COUNTED_ELEMENT_NAME)) {
                        ...
                        //ТОЛЬКО НОДУ СЧИТАЕМ ЗА РОДИТЕЛЯ!
                        parentElementName = startElementName;
                    }
                    try {
                        ElementProcessor currentProcessor = processors.get(startElementName);
                        ...
                        if(currentProcessor.elementIsProcessed() && startElementName.equals(TAG_ELEMENT)) {
                            //если это тег, и он содержит имя, нет смысла рассматривать остальные теги в этой ноде,
                            //до переходить сразу к следующей ноде!
